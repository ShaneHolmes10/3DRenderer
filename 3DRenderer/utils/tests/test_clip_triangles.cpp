...
static Vertex3 makeVertex(float x, float y, float z,
...
// Left frustum plane: focal*x + (W/2)*z >= 0  →  x + z >= 0 (with these params).
// At z=5, the left boundary is x = −5. Triangle entirely to the left.
TEST(ClipTriangles, AllVerticesOutsideLeftFrustum_ReturnsEmpty) {
    Triangle3 tri = makeTri(
        makeVertex(-8.0f, 0.0f, 5.0f),
...
TEST(ClipTriangles, OneVertexBehindNearPlane_ReturnsTwoTriangles) {
    // v2 is just barely behind the near plane; v0 and v1 are clearly in front.
    // All three are near the screen centre so the lateral frustum planes
    // leave the clipped intersection points untouched.
    Triangle3 tri = makeTri(
...
// S-H keeps one vertex and adds two intersection points: 3 vertices → 1 triangle.
TEST(ClipTriangles, TwoVerticesBehindNearPlane_ReturnsOneTriangle) {
    Triangle3 tri = makeTri(
        makeVertex( 0.0f, 0.0f,  1.0f),   // inside
...
// which satisfies d >= 0 and is treated as inside. The triangle should survive.
...
// A vertex at camera-space (0, 0, z) must project to the screen centre (W/2, H/2).
...
// x_screen = (x * focal / z) + W/2.  For (x=1, z=1): 1*100/1 + 100 = 200 = W.
// Verifies that focal length and perspective division are applied correctly.
...
// When the near plane cuts an edge, the new vertex should have a linearly
// interpolated colour between the two endpoints.
//
...
// All resulting vertex colours must remain within the range spanned by the
// input colours — clipping must never extrapolate outside [0, 200].
...
// Triangle mostly visible but with one vertex behind the near plane that,
// after clipping, also grazes the left frustum boundary.
// The visible portion is still non-trivial and must not be discarded.
TEST(ClipTriangles, StraddlingNearAndLeftFrustum_IsNonEmpty) {
    Triangle3 tri = makeTri(
        makeVertex(-3.0f, 0.0f,  5.0f),  // inside all planes
...
// A triangle whose vertices are simultaneously outside two frustum planes
// (left AND bottom here) should be completely clipped away.
...
// A very large triangle that completely contains the frustum pyramid should
// still clip down to a visible polygon — the output must be non-empty.
