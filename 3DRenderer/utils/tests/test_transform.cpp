...
TEST(Transform, ConstructWithPositionRotationDefaultScale)
{
    ...
    CHECK((t.getPosition() - pos).norm() < 0.01f);
    CHECK((t.getAngles() - rot).norm() < 0.01f);
    CHECK((t.getScale() - Eigen::Vector3f(1.0f, 1.0f, 1.0f)).norm() < 0.01f);
}
...
TEST(Transform, MultipleSettersWork)
{
    ...
    CHECK((t.getPosition() - Eigen::Vector3f(1.0f, 2.0f, 3.0f)).norm() < 0.01f);
    CHECK((t.getAngles() - Eigen::Vector3f(0.5f, 1.0f, 1.5f)).norm() < 0.01f);
    CHECK((t.getScale() - Eigen::Vector3f(2.0f, 3.0f, 4.0f)).norm() < 0.01f);
}
