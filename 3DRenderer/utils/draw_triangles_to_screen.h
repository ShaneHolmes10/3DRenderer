 ...
 *
 * Performs a depth test using the interpolated inverse-z value at each pixel.
 * Only fragments closer than what is already in the depth buffer are written.
 *
 * @param frame_buffer  The pixel buffer to write into.
 * @param depth_buffer  The depth buffer to test and update.
 * @param triangle      A screen-space triangle with 2D vertex positions, colours, and inverse-z values.
 */
