 ...
 *
 * Templated on PositionType so the same struct covers both 2D
 * screen-space vertices (Vertex2 = Vertex<Eigen::Vector2f>) and 3D
 * world/camera-space vertices (Vertex3 = Vertex<Eigen::Vector3f>).
 */
...
/**
 * @brief A triangle defined by three vertices with additional fields
 * for depth information.
 *
 * This struct inherits from the Triangle struct so it contains the same
 * fields as that but with the addition of the depth information per
 * vertex.
 */
template <typename PositionType>
