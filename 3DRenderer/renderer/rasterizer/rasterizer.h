...
void rasterize(const RasterTriangle& triangle, const TUniform& uniform,
    ...
    auto signedArea = [](const Eigen::Vector2f& A, const Eigen::Vector2f& B,
    ...
    for (int x_pixel_ind = x_start; x_pixel_ind < x_end; x_pixel_ind++) {
        for (int y_pixel_ind = y_start; y_pixel_ind < y_end; y_pixel_ind++) {
            ...
            // Compute the signed area of each sub-triangle formed by the pixel
            // and each edge. A pixel is inside the triangle when all three are
            // non-negative (counter-clockwise winding).
            ...
            // Barycentric weights: each weight is the sub-triangle area opposite
            // to that vertex divided by the total triangle area.
            ...
            // Interpolate all Varying fields across the triangle using the
            // barycentric weights, producing the per-fragment Varying.
            Varying varying;
            varying.position = {
                pixel.x(), pixel.y(),
