...
/**
 * @brief Handles the rendering pipeline from world space to screen space.
 * 
 * The Camera uses composition rather than inheritance. It does not exist
 * in the scene graph directly, but mounts to an Entity to inherit its
 * world transformation. This enables camera rigs where the camera
...
public:
     ...
     * then rasterizes each triangle with per vertex color interpolation.
     * 
