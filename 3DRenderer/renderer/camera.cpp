...
void Camera::draw(FrameBuffer& frame_buffer, DepthBuffer& depth_buffer,
                  const DrawCommand& draw_command) {
    const Entity* target_entity = draw_command.entity;
    const std::vector<Mesh>& target_meshes =
        target_entity->model->getMeshes();
    ...
    for (const Mesh& mesh : target_meshes) {
        ...
        for (const Face& face : mesh.getFaces()) {
            ...
            Eigen::Matrix4f view =
                this->mount->getWorldMatrix().inverse();
            ...
            // Build camera-space triangle, clip against frustum, and
            // draw
            Triangle3 tri3;
            tri3.vertex_A.position = v1_expressed.head<3>();
            tri3.vertex_A.color = vertices[face.v1].color;
            tri3.vertex_B.position = v2_expressed.head<3>();
            tri3.vertex_B.color = vertices[face.v2].color;
            ...
            for (const Triangle3& clipped : clipTriangle(
                     tri3, focal_length, width, height, NEAR_Z))
                drawTriangleToScreen(
                    frame_buffer, depth_buffer,
                    projectTriangle(clipped, focal_length, width,
                                    height));
        }
    }
}
