...
/**
 * @brief This class is responsible for facilitating the display of the
 * pixels to the screen.
 *
 * The view exists on a separate thread which allows us to have multiple
 * views as well as to facilitate the main rendering loop separate from
 * the viewport.
 *
 */
class Viewport {
   private:
    ...
    /**
     * @brief This is the frame mutex lock to prevent thread objects
     * from simultaneously accessing shared thread resources, thus
    ...
    /**
     * @brief This is the thread method that facilitates the window
     * function.
     *
     */
    void runWindow();
   ...
   public:
    ...
    /**
     * @brief This method needs to be called before hand to initialize
    ...
    /**
     * @brief This method sets the frame buffer of the view to be what
     * ever the input image is.
     *
    ...
    /**
     * @brief When done with the view we will need to join it back to
     * the main thread.
     *
     */
    void join();
