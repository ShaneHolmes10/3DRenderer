...
/**
 * @brief Represents an object in a 3D scene with hierarchical
 * transformations.
 *
 * Entity implements a scene graph pattern where objects can be
 * organized in parent-child hierarchies. Each entity maintains a local
 * transformation (relative to its parent) and computes a world
 * transformation (absolute position in the scene) by combining its
 * local transform with all parent transforms up the hierarchy.
 *
 * Scene Graph Example:
 * @code
 * World (at origin)
...
class Entity {
   private:
     ...
     *
     * Computed by recursively multiplying all parent world matrices
     * with this entity's local matrix. Updated automatically when
     * transforms change via setTransform(), applyTransform(), or
     * addChild().
     *
   ...
   public:
     ...
     *
     * Can be nullptr if this entity is just a transform node
     * (organizational node with no visual representation). Multiple
     * entities can share the same Model.
     */
    Model* model;
     ...
     *
     * Initializes the entity with no parent, no model and both local
     * and world transformation matrices set to identity (positioned at
     * origin with no rotation or scaling).
     */
    Entity()
        : parent(nullptr),
          model(nullptr),
     ...
     *
     * Recursively computes world matrices by combining this entity's
     * local matrix with its parent's world matrix, then updates all
     * children. Called automatically by setTransform(),
     * applyTransform(), and addChild().
     *
     * @note Should be called from entities whose parents have
     * up-to-date world matrices. Typically called automatically, but
     ...
     *
     * Establishes parent-child relationship and automatically updates
     * the child's world matrix (and all its descendants) to reflect the
     * new hierarchy. If child already has a parent, it should be
     * removed from the old parent first (currently not handled
     * automatically).
     *
    ...
    /**
     * @brief Set the local transformation, replacing the current
     * transform.
     *
     * Replaces this entity's local matrix with the matrix from the
     * given Transform, then updates world matrices for this entity and
     * all descendants.
     *
     ...
     * entity.setTransform(t);  // Entity now at (10, 0, 0) relative to
     * parent
     * @endcode
     */
     ...
     *
     * Returns the absolute transformation of this entity in world
     * space, computed by combining all parent transforms with this
     * entity's local transform.
     *
     * @return const Eigen::Matrix4f& Reference to world transformation
     * matrix
     */
    const Eigen::Matrix4f& getWorldMatrix() const {
        return world_matrix;
    }
     ...
     *
     * @return const Eigen::Matrix4f& Reference to local transformation
     * matrix
     */
    const Eigen::Matrix4f& getLocalMatrix() const {
        return local_matrix;
    }
