...
TEST(Entity, ParentChildRelationship) {
    ...
    Transform t_parent(Eigen::Vector3f(100, 0, 0),
                       Eigen::Vector3f::Zero());
    parent.setTransform(t_parent);

    Transform t_child(Eigen::Vector3f(50, 0, 0),
                      Eigen::Vector3f::Zero());
    child.setTransform(t_child);

    parent.addChild(child);

    Eigen::Vector3f child_world_pos =
        child.getWorldMatrix().block<3, 1>(0, 3);
...
TEST(Entity, AddChildAutoUpdates) {
    ...
    Transform t_child(Eigen::Vector3f(30, 0, 0),
                      Eigen::Vector3f::Zero());
    child.setTransform(t_child);

    parent.addChild(child);

    Eigen::Vector3f child_pos =
        child.getWorldMatrix().block<3, 1>(0, 3);
...
TEST(Entity, ModifyingParentUpdatesChild) {
    ...
    Transform t_child(Eigen::Vector3f(10, 0, 0),
                      Eigen::Vector3f::Zero());
    child.setTransform(t_child);
...
TEST(Entity, ScalePropagation) {
    ...
    Transform t_parent(Eigen::Vector3f::Zero(), Eigen::Vector3f::Zero(),
...
TEST(Entity, ApplyTransformWithRotation) {
    ...
    Transform t2(Eigen::Vector3f::Zero(),
                 Eigen::Vector3f(0, M_PI / 2, 0));
    e.applyTransform(t2);
...
TEST(Entity, MultipleChildren) {
    ...
    CHECK((pos1 - expected1).norm() < 0.1f &&
          (pos2 - expected2).norm() < 0.1f);
}
...
TEST(Entity, SolarSystemExample) {
    ...
    Transform t_sun(Eigen::Vector3f(100, 0, 0),
                    Eigen::Vector3f::Zero());
    sun.setTransform(t_sun);

    Transform t_earth(Eigen::Vector3f(50, 0, 0),
                      Eigen::Vector3f::Zero());
    earth.setTransform(t_earth);

    Transform t_moon(Eigen::Vector3f(10, 0, 0),
                     Eigen::Vector3f::Zero());
...
TEST(Entity, LocalMatrixIndependentOfParent) {
    ...
    Eigen::Vector3f child_local_pos =
        child.getLocalMatrix().block<3, 1>(0, 3);
...
TEST(Entity, MultiLevelRotation) {
    ...
    Transform t_root(Eigen::Vector3f::Zero(),
                     Eigen::Vector3f(0, M_PI / 4, 0));
    root.setTransform(t_root);

    Transform t_middle(Eigen::Vector3f(10, 0, 0),
                       Eigen::Vector3f(0, M_PI / 4, 0));
